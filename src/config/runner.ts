...
import { readConfSync, generateId, readJsonSync, lodash } from "@/utils";
...
class Runner implements Omit<Options, ""> {
  ...
  async configure(): Promise<Config> {
    ...
    if (this.cacheActions) {
      actionCache = this.actionsOffline
        ? new ActionCacheOffline(this.actionsPath)
        : new ActionCache(this.actionsPath);
    }
