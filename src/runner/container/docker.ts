...
class DockerContainer extends Container {
  ...
  async context() {
    ...
    return {
      ...
      ports: Object.fromEntries(
        Object.entries(Ports).map(([containerPort, HostInfos]) => {
  ...
  pullImage(force?: boolean) {
    return new Executor(async () => {
      const { image, platform, authconfig, pull } = this.options;
    ...
    return Executor.Pipeline(
      ...
      new Executor(async () => {
        ...
        if (Object.keys(networkInspectInfo?.Containers || {}).length === 0) {
          ...
          logger.debug(
            "\u{1F433}",
            `Refusing to remove network ${name} because it still has active endpoints`,
          );
        }
      }),
    );
  ...
  private createContainer() {
    return new Executor(async () => {
      ...
      logger.debug(
        "\u{1F433}",
        `Created container name=${options.name} id=${container.id} from image ${options.image} (platform: ${options.platform || ""})`,
      );
  ...
  spawnSync(command: string, args: string[], options: ContainerExecOptions = {}) {
    const { container } = this;
    const { cwd, env, privileged, user } = options;
  ...
  static Setup(runner: Runner) {
    return new Executor(() => {
      ...
      runner.services = Object.fromEntries(
        Object.entries(runner.run.job.services || {}).map(([serviceId, service]) => {
      ...
      runner.cleanContainerExecutor = Executor.Pipeline(
        ...
        new Executor(async () => {
          ...
          if (createAndDeleteNetwork) {
            ...
            logger.info(
              "\u{1F433}",
              `Cleaning up network for Job: ${runner.name}, and network name is: ${networkName}`,
            );
