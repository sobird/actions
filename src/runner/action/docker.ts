    ...
    return new Executor(async (ctx) => {
      ...
      if (DockerAction.IsDockerfile(image)) {
        ...
        try {
          ...
          if (
            (!runner.config.containerPlatform || runner.config.containerPlatform === platform) &&
            !runner.config.rebuild
          ) {
            logger.debug(
      ...
      if (entrypointPath) {
        ...
        entrypoint = shellQuote
          .parse(this.runs[entrypointStage], (name) => {
            return `\${${name}}`;
          })
          .map((part) => {
            if (typeof part === 'object' && (part as { op: ControlOperator }).op) {
