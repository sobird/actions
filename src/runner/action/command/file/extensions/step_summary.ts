...
const CreateStepSummaryCommand: FileCommandExtension = {
  ...
  async process(runner, filename) {
    ...
    if (filesize > AttachmentSizeLimit) {
      console.error(
        util.format(
          Constants.Runner.UnsupportedSummarySize,
          AttachmentSizeLimit / 1024,
          filesize / 1024,
        ),
      );
      console.info(
        `Step Summary file (${filename}) is too large (${filesize} bytes); skipping attachment upload`,
      );
      return;
    }
    ...
    file.on("end", () => {
      ...
      if (runner.context.vars["system.github.results_endpoint"]) {
        console.log(
          `Queueing results file (${filename}) for attachment upload (${attachmentName})`,
        );
