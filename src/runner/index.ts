...
class Runner {
  ...
  constructor(
    ...
    const runId = createFnv1aHash(
      `${context.github.repository}${context.github.workflow}${context.github.workflow_sha}`,
    ).toString();
  ...
  async addMatchers(config: IssueMatchersConfig) {
    await Executor.Mutex(
      new Executor(() => {
        ...
        newMatchers.push(
          ...existingMatchers.filter((x) => {
            return !newOwners.has(x.owner);
          }),
        );
