...
async function daemonAction(opts: Options, program: typeof Command.prototype) {
  ...
  const labels = new Labels(
    appconf.runner.labels.length > 0 ? appconf.runner.labels : registration.labels,
  );
  ...
  try {
    ...
    if (connectError.code === Code.Unimplemented) {
      logger.error(
        "Your Gitea version is too old to support runner declare, please upgrade to v1.21 or later",
      );
      return;
    }
