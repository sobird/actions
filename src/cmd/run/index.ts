...
export const runCommand = new Command("run")
  ...
  .addOption(
    new Option("--no-skip-checkout", "do not skip actions/checkout").conflicts("bindWorkdir"),
  )
  ...
  .option(
    "--token <token>",
    "if you want to use private actions on GitHub, you have to set personal access token",
  )
  .option(
    "--env <envs...>",
  ...
  .option(
    "--vars-file <path>",
    "file with list of vars to read from (e.g. --vars-file .vars)",
    ".vars",
  )
  .option(
    "--inputs <inputs...>",
  ...
  .option(
    "--secrets <secrets...>",
    "secret to make available to actions with optional value (e.g. --secrets mysecret=foo,toke=bar)",
    collectObject,
  )
  .option(
    "--secrets-file <path>",
    "file with list of secrets to read from (e.g. --secrets-file .secrets)",
    ".secrets",
  )
  ...
  .option(
    "--actions-cache-addr <addr>",
    "the address to which the actions/cache server binds.",
    ip.address(),
  )
  .option(
  ...
  .option(
    "--actions-path <path>",
    "the dir where the actions get cached",
    path.join(ACTIONS_HOME, "actions"),
  )
  .option(
    "--actions-offline",
  ...
  .option(
    "--labels <labels...>",
    "custom image to use per platform (e.g. --labels ubuntu-latest=nektos/act-environments-ubuntu:18.04)",
    collectArray,
  )
  ...
  .option(
    ...
    "specify the network to which the container will connect",
  )
  .option(
