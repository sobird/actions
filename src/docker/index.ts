...
export class Docker extends Dockerode {
  async pullImage(repoTag: string, inputs: PullImageInputs = {}) {
    ...
    try {
      ...
      logger.error(
        "Unable to determine if image already exists for image '%s' (%s): %s",
        repoTag,
        inputs.platform,
  ...
  static SocketAndHost(containerSocket: string = "") {
    ...
    if (!dockerHost && socketHost.socket && !Docker.isHostURI(socketHost.socket)) {
      ...
      logger.error(
        `DOCKER_HOST was not set, couldn't be found in the usual locations, and the container daemon socket ('${socketHost.socket}') is invalid`,
      );
    }
